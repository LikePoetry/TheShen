...
#include "../src/Interfaces/IThread.h"
...
static void wipeWithPattern(sAllocUnit* allocUnit, uint32_t pattern, const unsigned int originalReportedSize)
{
	...
	uint32_t* pre = (uint32_t*)(allocUnit->actualAddress);
	uint32_t* post = (uint32_t*)(((char*)allocUnit->actualAddress) + allocUnit->actualSize - paddingSize * sizeof(uint32_t));
...
static void dumpAllocations(FileStream* fh)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		{
			dumpLine(
				fh, "% 6d 0x%016zX 0x%08zX 0x%016zX 0x%08zX 0x%08X %-8s    %c       %c    %s", ptr->allocationNumber,
...
{
	{
		...
		dumpLine(&fh, " ------------------------------------------------------------------------------");
		dumpLine(
			&fh, "|                Memory leak report for:  %02d/%02d/%04d %02d:%02d:%02d                  |", tme.tm_mon + 1, tme.tm_mday,
...
bool mmgrValidateAllocUnit(const sAllocUnit* allocUnit)
{
	...
	for (unsigned int i = 0; i < paddingSize; i++, pre++, post++)
	{
		...
		m_assert(*pre == (uint32_t)(prefixPattern));

		if (*post != (uint32_t)(postfixPattern))
		{
		...
		m_assert(*post == (uint32_t)(postfixPattern));
	}
